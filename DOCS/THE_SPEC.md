...
- `lib/tests/repo_test.sh` — repo-wide invariants: every tracked shell script parses (POSIX files under `dash`, not `bash -n`, which accepts `[[ ]]` and proves nothing), every skill has a `SKILL.md` with matching frontmatter, and every skill sits where a hotwired tool looks for it. These were inline CI steps; a check you cannot run before pushing is a slow way to learn something a suite tells you in a second.
- `lib/tests/assert.sh` — shared assertion helpers, and the reason they exist: nine assertions in one session could not fail. `check_control` refuses a check whose control also passes; `code_of` strips comments and quoted spans before searching a file for a construct it documents avoiding; `read_lines_into` replaces `mapfile`, which is bash 4+ and silently yields an empty array on macOS's bash 3.2, skipping whatever loop depended on it.
...
| `build [--out <file>] [--title <text>] [--force]` | Write an EMPTY DRAFT skeleton in `DOCS/` where it exists, else the repo root. No git evidence: that is the builder's account of its own work |
| `ensure [--dir] [--out] [--title]` | `find`, else `build`; exit 3 means the spec found or written is still a DRAFT needing filling |
| `list [--dir]` | Every spec-shaped file in the project — a project may have several and only the human knows which governs |
